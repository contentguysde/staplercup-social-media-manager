...
async function getWebhookComments(credentials: { accountId: string; accessToken: string }): Promise<any[]> {
  try {
    ...
    // Only verify comments that are:
    // 1. Less than 7 days old
    // 2. NOT from our own account (own replies don't need verification)
    const recentExternalComments = result.rows.filter(row => {
      ...
      // Skip verification for our own comments (replies from our account)
      const isOwnComment = row.from_id === credentials.accountId ||
                           row.from_username === 'staplercup_official';
